...
3. 시스템 변수 탭에서 새로 만들기를 클릭하고, 값을 입력해줍니다.
  1. 변수 이름 : `JAVA_HOME`
  2. 변수 값 : `C:₩Program Files₩Zulu₩zulu-11`
  3. 시스템 변수에 방금 설정한 환경 변수가 제대로 만들어졌는지 확인
...
6. 방금 전 JAVA_HOME을 만들었던 것과 동일한 방법으로 아래의 환경 변수를 추가해주세요.
  1. 변수 이름 : CLASSPATH
  2. 변수 값 : `%JAVA_HOME%₩lib`
7. 환경 변수 설정이 제대로 완료되었는지 확인합니다.
  1. 작업표시줄의 검색바에서 cmd를 검색하여 명령 프롬프트를 실행한 후, path를 입력하고 엔터를 눌러줌
